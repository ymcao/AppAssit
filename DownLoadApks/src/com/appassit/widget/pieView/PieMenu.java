...
public class PieMenu extends FrameLayout {
	...
	public interface PieView {
		...
		public void layout(int anchorX, int anchorY, boolean onleft, float angle, int parentHeight);
	 ...
	 * 
	 * @param angle
	 *            from 0..PI to Android degrees (clockwise starting at 3 o'clock)
	 * @return skia angle
	 */
		...
		if (MotionEvent.ACTION_DOWN == action) {
			...
			if (item == null) {
				...
				if ((item != null) && item.isPieView() && (item.getView() != null)) {
					int cx = item.getView().getLeft() + (onTheLeft() ? item.getView().getWidth() : 0);
					int cy = item.getView().getTop();
					mPieView = item.getPieView();
					layoutPieView(mPieView, cx, cy, (item.getStartAngle() + item.getSweep()) / 2);
				}
				invalidate();
			}
		}
	...
	private boolean inside(PointF polar, float offset, PieItem item) {
		return (item.getInnerRadius() - offset < polar.y) && (item.getOuterRadius() - offset > polar.y) && (item.getStartAngle() < polar.x)
