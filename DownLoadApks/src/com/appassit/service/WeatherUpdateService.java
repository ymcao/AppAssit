...
import com.appassit.activitys.MainAppActivity_;
...
import com.appassit.tools.DateUtil;
...
public class WeatherUpdateService extends Service {
	...
	public void onCreate() {
		...
		new Timer().scheduleAtFixedRate(new TimerTask() {
			@Override
			public void run() {
				getWeatherFormNet(getNextWeatherCode(SLAppication.getCurCityIndex()));
			}
	...
	private String getNextWeatherCode(int index) {
		return SLAppication.getMyArea().get(index % SLAppication.getMyArea().size()).getWeatherCode();
	}
	...
	private void getWeatherFormNet(String id) {
		executeRequest(new GsonWeatherRequest(Const.WEATER_INFO_URL + "?app=tq360&_jsonp=renderData&code=" + id, SunModel.SunRequestData.class, responseListener(),
				errorListener()));
	}
	...
	void updateTime() {
		...
		remoteView.setOnClickPendingIntent(R.id.rl_provider_widget, PendingIntent.getActivity(this, 0, new Intent(this, MainAppActivity_.class), 0));
	...
	private BroadcastReceiver mTimePickerBroadcast = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(ACTION_UPDATE_WEATHER)) {
				...
				getWeather(getNextWeatherCode(SLAppication.getCurCityIndex()));
			} else if (intent.getAction().equals(ACTION_NEXT_CITY)) {
				// LogUtil.i(TAG, "next city");
				SLAppication.setCurCityIndex((SLAppication.getCurCityIndex() + 1) % SLAppication.getMyArea().size());
				getWeather(getNextWeatherCode(SLAppication.getCurCityIndex()));
			} else {
				updateTime();
			}
